...
from decision_engine import decide, load_config, persist_decision
from models import Approval, EnrichmentResult, Event, init_db
from responder import execute_response, init_lab_allowlist


def main():
    """Run the end-to-end block approval demonstration."""
    ...
    try:
        ...
        if response["status"] != "simulation":
            raise RuntimeError(f"TEST FAILED: responder returned {response['status']}")
